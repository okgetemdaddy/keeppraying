...
export function useAccountabilityCircles() {
  const { user } = useAuth();
  const [circles, setCircles] = useState<(Circle & { memberCount: number })[]>([]);
    ...
    const withCounts = await Promise.all(
      circleData.map(async (c: any) => {
        const { count } = await supabase
          ...
          .eq("circle_id", c.id);
        return { ...c, memberCount: count ?? 0 } as Circle & { memberCount: number };
      })
    );
...
export function useCircleDetail(circleId: string | undefined) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [circleRes, membersRes, prayersRes, encourageRes] = await Promise.all([
    ...
    if (membersRes.data) {
      ...
      // Compute positive aggregate stats
  ...
  return { circle, members, prayers, encouragements, loading, stats, refetch: fetchAll };
}
