...
export default function KeepReadingAuth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "forgot") {
        ...
        if (error) throw error;
        toast({ title: "Welcome to KeepRead.ing 📖", description: "Check your email to confirm your account." });
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        <img src={heroBg} alt="Bible reading" className="absolute inset-0 w-full h-full object-cover" />
        <div className="hero-overlay absolute inset-0" />
        <div className="relative z-10 flex flex-col justify-center items-center p-12 text-center">
          <Link to="/" className="font-display text-4xl font-bold text-white mb-4">KeepRead.ing</Link>
          <p className="text-white/80 text-lg max-w-xs font-display italic leading-relaxed">
            "Your word is a lamp for my feet, a light on my path."
          </p>
          <p className="text-white/60 mt-3 text-sm">— <VerseLink reference="Psalm 119:105" className="[&_.verse-text]:text-white/60 [&>span]:bg-white/10 [&>span]:border-white/20" /></p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center">
            <Link to="/" className="font-display text-3xl font-bold text-foreground">KeepRead.ing</Link>
            <p className="mt-2 text-muted-foreground">
              {mode === "forgot" ? "Reset your password." : mode === "signin" ? "Welcome back, reader." : "Start your reading journey."}
            </p>
          </div>
